// Datenstrukturen für User, Aufgaben & Abgaben
type User = {
    id: string;
    name: string;
    points: number;
...
// Punkte berechnen
function calculatePoints(deadline: Date, submissionTime: Date): number {
    const base = 10;
...
// Abgabe der Aufgabe 
function submitTask(user: User, task: Task, submissionTime: Date) {
...
// Feedback anzeigen
function showFeedback(user: User, task: Task, points: number) {
...
// Rangliste anzeigen
function renderLeaderBoard(userList: User[]) {
...
// Level berechnen
function calculateLevel(user: User): number {
    // Hoher Rang
    if (user.points >= 100) {
        return 3;
    }
...
function saveData(userList: User[]) {
